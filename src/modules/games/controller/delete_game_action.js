...
export const deleteGameAction = (database, websockets) => async (req, res) => {
  ...
  if (requestingUser.role !== 'admin' && game.owner_id !== requestingUser.id) {
    logger.warn('Unauthorized attempt to delete game');
    return jsonRes(res, 'Unauthorized', [], 401);
  }
  ...
  const updatedHostedGames = ownerUser.hosted_games.filter((g) => g !== gameId);
