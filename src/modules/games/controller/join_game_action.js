...
export const joinGameAction = (database, websockets) => async (req, res) => {
  ...
  if (!gameData) {
    logger.warn('User tried to join non-existent game');
  ...
  if (gameData.owner_id === requestingUser.id) {
    ...
    return jsonRes(res, 'Player cannot join game hosted by himself.', [], 404);
  }
